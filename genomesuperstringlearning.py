...
# print revcomp(s[-1])
# print s[1:]
# print map(revcomp,s[0:-1])

print revcomp(extend(revcomp(s[-1]), map(revcomp,s[0:-1])))#[:-len(s[0])]

print extend(s[0], s[1:])
...
The extend function then outputs the extended/completed consensus, which is
reverse complemented to get back the consensus based on the original sequences
